...
func init() {
	...
	rootCmd.PersistentFlags().StringVarP(&containerName, "containerName", "c", "-1", "The specific containerName name to trail from")
...
func runPodtail(cmd *cobra.Command, args []string) {
	...
	for _, pod := range pods {
		...
		for _, container := range containers {
			if containerName == "" || strings.Contains(container, containerName) {
				t := tailInfo{
					pod:           pod,
					container:     container,
					since:         since,
